...
export default function HomePage() {
  ...
  // ── Deep-link: ?select=osmId&lat=...&lon=...
  // Generated by the /favorites page when a user clicks "Open on map".
  // 1. Fly the map to the coordinates immediately.
  // 2. Once restaurants are loaded, find the matching place and select it.
  ...
  useEffect(() => {
    ...
    if (!selectId || isNaN(lat) || isNaN(lon)) return;
    if (deepLinkHandled.current) return;

    // Step 1 — fly the map (runs as soon as the component mounts)
  ...
  // Step 2 — once filteredPlaces loads, find and auto-select the target place
  useEffect(() => {
    const selectId = searchParams.get("select");
    if (!selectId || deepLinkHandled.current) return;
    if (loading || filteredPlaces.length === 0) return;

    const target = filteredPlaces.find(p => p.osm_id === decodeURIComponent(selectId));
    if (!target) return;

    deepLinkHandled.current = true;
    setSelectedPlace(target);
    if (userLocation) doRoute(target, routeMode);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // Returns "ok" | "auth_required" | "error" — we act on auth_required.
  const handleToggleFavorite = useCallback(async (place: PlaceCard) => {
    ...
    if (result === "auth_required") {
      setShowAuthModal(true);
      toast.info("Sign in to save your favourite spots");
      return;
    }
    if (result === "error") {
  ...
  return (
    <div style={{ position:"fixed",inset:0,display:"flex",flexDirection:"column",background:"var(--surface-0)",fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <header style={{
        ...
        {/* Search bar — filters the already-loaded results by name/cuisine.
            NOT a location search. Tooltip + placeholder clarify this to avoid
            users typing city names expecting map navigation. */}
        <div style={{ flex:1,maxWidth:340,position:"relative" }}>
          ...
          <input
            ...
            title="Filters the restaurants already visible on the map. To search a new location, move the map and tap 'Search this area'."
            value={nameQuery}
      ...
      <div style={{ flex:1,display:"flex",overflow:"hidden",position:"relative",marginTop:pinDropActive?34:0,transition:"margin-top 200ms var(--ease-out)" }}>
        ...
        <div style={{
          ...
          <div style={{ maxHeight:showFilters?580:0,overflow:"hidden",transition:"max-height 380ms var(--ease-out)",flexShrink:0 }}>
            <div style={{ borderBottom:"1px solid rgba(28,25,23,0.07)" }}>
              <FiltersPanel filters={filters} onChange={handleFilters} places={filteredPlaces}/>
            </div>
          </div>
        ...
        <div style={{ flex:1,position:"relative",overflow:"hidden" }}>
          <ErrorBoundary
            fallback={
              <div style={{ position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:14,background:"var(--surface-0)",padding:24 }}>
                <div style={{ width:52,height:52,borderRadius:16,background:"rgba(224,90,30,0.08)",border:"1px solid rgba(224,90,30,0.16)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24 }}>🗺️</div>
                <div style={{ textAlign:"center" }}>
                  <p style={{ margin:"0 0 6px",fontSize:14,fontWeight:800,color:"var(--ink-1)",letterSpacing:"-0.03em" }}>Map failed to load</p>
                  <p style={{ margin:"0 0 18px",fontSize:12,color:"var(--ink-3)" }}>Try reloading the page — your saved places are safe.</p>
                  <button onClick={()=>window.location.reload()} style={{ padding:"10px 22px",borderRadius:10,border:"none",background:"var(--brand)",color:"white",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",boxShadow:"0 4px 14px rgba(224,90,30,0.25)" }}>
                    Reload
                  </button>
                </div>
              </div>
